...
const TestPage: React.FC = () => {
  ...
  useEffect(() => {
    const testPageData = localStorage.getItem('testPageData');
    if (testPageData) {
      setResults(JSON.parse(testPageData));
      localStorage.removeItem('testPageData');
    }
  }, []);
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: '#181825', py: 6, overflowY: 'auto', width: '100vw', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <Container maxWidth={false} disableGutters sx={{ width: '100vw', px: { xs: 1, md: 6 }, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <Typography variant="h4" sx={{ color: '#b4befe', fontWeight: 800, mb: 4, letterSpacing: 1, textAlign: 'center' }}>
          {results ? 'Calculation Steps' : 'Test: All Steps Preview (API)'}
        </Typography>
        {!results && (
          <Box component="form" onSubmit={handleSubmit} sx={{ mb: 6, background: '#232136', borderRadius: 3, p: 4, boxShadow: '0 2px 16px 0 #18182588', border: '1.5px solid #313244', width: '100%', maxWidth: 1400, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Stack spacing={3} direction={{ xs: 'column', sm: 'row' }} sx={{ width: '100%', justifyContent: 'center', alignItems: 'center' }}>
              <TextField
                ...
                required
                sx={{ input: { color: '#cdd6f4' }, label: { color: '#a6adc8' }, flex: 1, minWidth: 180 }}
              />
              <TextField
              ...
              <TextField
                ...
                placeholder="e.g. 1,3"
                sx={{ input: { color: '#cdd6f4' }, label: { color: '#a6adc8' }, flex: 2, minWidth: 220 }}
              />
              <Button variant="contained" component="label" sx={{ background: '#89b4fa', color: '#181825', fontWeight: 700, flex: 1, minWidth: 160 }}>
                Upload CSV
                <input type="file" accept=".csv" hidden onChange={handleFileUpload} />
              </Button>
              <Button type="submit" variant="contained" sx={{ background: '#a6e3a1', color: '#181825', fontWeight: 700, flex: 1, minWidth: 160 }}>
                {loading ? <CircularProgress size={24} sx={{ color: '#181825' }} /> : 'Run Test'}
              </Button>
            </Stack>
            {csvFile && <Typography sx={{ color: '#a6adc8', mt: 2 }}>Selected file: {csvFile.name}</Typography>}
        ...
        <Box sx={{ width: '100%', maxWidth: 1400, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {results && (
            <>
              ...
              {getStep(17) && <Step17Summary data={getStep(17)} />}
            </>
          )}
        </Box>
      </Container>
